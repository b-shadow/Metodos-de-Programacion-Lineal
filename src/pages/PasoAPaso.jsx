...
const PasoAPaso = () => {
    ...
    const [cVec, setCVec] = useState([]); // Cj y Cb
    ...
    useEffect(() => {
        ...
        const n = c.length;
        const EPS = 1e-9;        // tolerancia numérica
        const totalVars = n + m; // X + S

        while (iter++ < maxIter) {
            // 1) Columna pivote: buscar el más NEGATIVO en toda la fila Z (X y S)
            const rc = tablaLocal[0].slice(1, 1 + totalVars); // Z sin Cb ni RHS
            let indCol = -1;
            let minValor = -EPS;   // umbral: valores >= -EPS se consideran no-negativos

            rc.forEach((v, idx) => {
                if (v < minValor) {
                    minValor = v;
                    indCol = idx + 1;  // +1 por la columna Cb al inicio
                }
            });

            // Si no hay negativos en ninguna columna de decisión ni de holgura → óptimo
            if (indCol === -1) {
                setSolucionOptima(true);
                break;
            }

            // Nombre correcto de la columna (puede ser Xk o Sk)
            const isSlack = indCol > n;
            const nombreCol = isSlack
                ? sx[indCol - n - 1].toUpperCase()
                : vx[indCol - 1];

            // 2) Fila pivote: prueba de razón mínima
